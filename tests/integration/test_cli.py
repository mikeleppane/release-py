...
class TestCLIVersion:
    ...
    def test_version_flag(self):
        ...
        assert "release-py" in result.stdout
...
class TestCLIInit:
    ...
    def test_init_creates_config(self, tmp_path: Path):
        """init creates release-py config in pyproject.toml."""
        ...
        assert result.exit_code == 0
        assert "release-py" in pyproject.read_text()
        assert "[tool.release-py]" in pyproject.read_text()
    ...
    def test_init_creates_workflow(self, tmp_path: Path):
        ...
        assert "release-py" in workflow_path.read_text()
